...
int main (int argc, char **argv)
{
  ...
  char                			*web_service = NULL;
  char                          *uqi = NULL;
  char                			contact_string[256];
  struct idss__get_file_response   	response;
  char					filename[1024];
  int					j;
  ...
  while ((j = getopt (argc, argv, "h:p:q:vu")) > 0)
  {
    switch (j)
    {
      case 'h':
  ...
  if (!uqi)
	{
	 printf("You must specify a query UQI.\n");
	 if (web_service)
    free (web_service);
	 return 0;
	}
  ...
  if (soap_call_idss__get_file(&soap, contact_string, "", uqi, &response))
  	{
    	 soap_print_fault (&soap, stderr);
  	}
  else
	{
	 if (response.status == 4)
		...
		printf("Unable to find the related file on the server.\n");
	 else if (response.status == 2)
		printf("Query not completed yet. Please retry later.\n");
	 else if (response.status == 1)
		printf("Unable to find a query with the specified UQI.\n");
	 else if (response.status == 0)
		{
		 if (response.result)
			{
		 	 snprintf(filename, 1024, "%s.xml", uqi);
			 save_dime_data(response.result, filename);
			 printf("Filename: %s\n", filename);
		 	}
		}
	}
