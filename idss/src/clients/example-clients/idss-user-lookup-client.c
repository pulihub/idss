/** @file idss-user-lookup-client 
 * Lookups an existing user.
 *
 * To run
 *      ./idss-user-lookup-client
 *
...
{
  printf ("%s: Usage:\n", name);
  printf ("\t-s user DN: distinguished name of the user you want to lookup\n");
...
/* Setting default values */
  ...
  if (verbose)
  {
    ...
    if (name)
    {
      printf ("Looking up user's DN... %s\n", name);
    }
    else
    {
      printf ("Looking up any available user's DN\n");
    }
  }

/*
 * soap_call_idss__lookup_user
 * 
 * lookups the information about the user with the specified DN.
 * It contacts the supplied MDS server on the default port
 * 
 * name: the DN mapped on the user to be searched
 * result: query result 
 */
  if (soap_call_idss__lookup_user
  ...
  else
  {
    if (verbose)
    {
      printf ("Your user's DN has been looked up\n");
    }
    if(result)
      save_dime_data(result, "user.xml");
