...
extern void xeq_all_pthreads(const char* file, const uint line);

extern pthread_attr_t G_joinable, G_detached;

#if defined(D1HT_PTHREAD_LIGHT)

#define D1HT_PTHREAD_NAME_SIZE 4
...
#endif
#if defined(XEQP_THREAD)
        xeq_last.xeqpoint_init();
        xeq_caller.xeqpoint_init();
#endif
    }
};
#define PTHREAD_TEST_CREATE(THREAD_ROUTINE, PARM, STATE, XEQ_FLAG) ({										\
	pthread_t		thread_id;																				\
	void*			_PARM = PARM;																			\
	int errsave = PTXEQ(pthread_create( &thread_id, STATE, &THREAD_ROUTINE, (void *) _PARM), XEQ_WARNING);	\
	if (errsave != 0) {																						\
		G_stats.total.stats[stat_memshort].count++;															\
		mysleep_msecs(500); /* we may be short of memory */													\
		errsave = PTXEQ(pthread_create( &thread_id, STATE, &THREAD_ROUTINE, (void *) _PARM), XEQ_FLAG);		\
		if (errsave != 0) thread_id = 0;																	\
	}																										\
	if (thread_id != 0) VERBOSE(stderr, "Thread %s created (ptid=%u)\n", #THREAD_ROUTINE, (uint) thread_id);\
	errno = errsave;																						\
	thread_id;																								\
})

#define PTHREAD_INIT 				{ pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL); (void) _parm_temp; }

#define PTHREAD_MAYLOCK 
#define PTHREAD(THREAD_ROUTINE) 	void* THREAD_ROUTINE(void* _parm_temp) 
#define PTHREAD_PARM_ASSIGN(PARM) 	PARM = (typeof(PARM)) _parm_temp
#define PTHREAD_PARM_FREE 			{ FREE(_parm_temp); }
#define PTHREAD_DETACH  			pthread_detach(pthread_self());

#define PTHREAD_PARM_COPY(PARM) {																			\
	if (_parm_temp == NULL) EXIT("Invalid thread parm");													\
	PARM = *((typeof(PARM) *) _parm_temp);																	\
}
...
#define PTHREAD_CREATE(THREAD_ROUTINE, PARM, STATE) PTHREAD_TEST_CREATE(THREAD_ROUTINE, PARM, STATE, XEQ_ERROR)

extern pthread_t G_TCP_thread_id, G_UDP_thread_id, G_SIG_thread_id;
extern pthread_data_t *G_pthread_data_header;
extern pthread_key_t G_pthread_data_key;
