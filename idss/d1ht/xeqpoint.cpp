...
xeqpoint_t 	G_xeqpoint;
bool		G_xeq = true;

#if defined(XEQP) 

void xeqpoint_t::xeq_notime(const char* filename, const uint linenumber) {

	if (G_exiting || !G_xeq || !G_started) return;

	strncpy(file, filename, XEQ_NAME_SIZE);
	line 			= linenumber;
	tv_xeq.tv_sec	= 0;
}

void xeqpoint_t::xeq(const char* filename, const uint linenumber) {

	if (G_exiting || !G_xeq || !G_started) return;

	strncpy(file, filename, XEQ_NAME_SIZE);
	line 	= linenumber;
	gettimeofday(&tv_xeq, NULL);

	// VERBOSEX(stderr, "XEQPOINT FILE=%s, LINE=%d\n", filename, linenumber);
}
#endif
