...
#include "d1ht_h.h"
#if defined(DEBUG)
#warning "This program is being compiled with the DEBUG flag turned on, which severely hurts its performance and"
#warning "ability to cope with workloads. Because of that, any D1HT executable generated with the DEBUG flag turned on"
#warning "MUST NOT be used to gather any statistics of any type."
#endif

int main(int argc, char** argv) {
    
    d1ht::d1ht_t d1ht(argc, argv);

    sigset_t blockmask;

    if (sigfillset(&blockmask) < 0) {
        int errsave = errno;
        fprintf(stderr, "D1HT ERROR Cause: call to sigfillset failed, msg=%s\n", strerror(errsave));
        exit(EXIT_FAILURE);
    }
    sigsuspend(&blockmask); // should block forever

    fprintf(stderr, "ERROR Cause: Unexpected program exit\n"); // should never reach this point
}
