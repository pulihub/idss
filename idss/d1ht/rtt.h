 ...
 *		smoothed RTT plus X times the deviation
 */

class rtt_info {
  float		srtt;			/* smoothed RTT estimator, microseconds */
  float		rttvar;			/* smoothed mean deviation, microseconds */
  lock_t	rtt_mutex;
  bool		inited;

  void		update_rtt(const float, const float);
  timeval 	rto(bool lookup);
...
public:
  ...
  rtt_info() {
  	...
  	LOCK_INIT(rtt_mutex, maychain=true, 0);
  }
  
  void	rtt_init(const NUM_32 i_srtt, const NUM_32 i_rttvar) { 
	  srtt 	= (float) i_srtt;
	  rttvar= (float) i_rttvar;
	  inited= true;
  }
  
  float 	rtt() 			{ return srtt; 	};
  float 	rttv() 			{ return rttvar;};

  timeval 	rto_lookup() 	{ return rto(true);  }  
  timeval 	rto() 			{ return rto(false); }

  void		update_rtt(const timeval);
  void		update_rtt(const timeval, const timeval);
};

extern rtt_info G_rtt_msg;
extern rtt_info G_rtt_lookup;
