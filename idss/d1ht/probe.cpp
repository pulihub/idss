...
namespace d1ht
{
// the probe_pthread routine bellow was created solely because I was not able to create a pthread that was a memer of a structure
PTHREAD(probe_pthread) {
	PTHREAD_DETACH;
	PTHREAD_INIT;
	PTHREAD_MAYLOCK;
	
	IPaddr host;  
	PTHREAD_PARM_COPY(host);
	
	if (G_parms.myIP != host && !G_exiting) G_probe.probe_detached(host);
	
	PTHREAD_PARM_FREE;
	return NULL;
}

PTHREAD(probe_and_propagate) {
	PTHREAD_DETACH;
	PTHREAD_INIT;
	PTHREAD_MAYLOCK;
	
	events_t event;  
	PTHREAD_PARM_COPY(event);
	
	int itype;
	
	if (G_probe.probe_host(event.peer)) {
		itype = JOINS;
	} else {
		itype = LEAVES;
	}
	
	G_RT.ack_event( event.peer, event.TTL, itype );
