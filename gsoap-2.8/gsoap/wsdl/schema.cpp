  ...
  if (targetNamespace && (!schema.targetNamespace || strcmp(targetNamespace, schema.targetNamespace)))
  {
    if (!Wflag)
    {
      if (schema.targetNamespace)
        fprintf(stderr, "\nWarning: attempt to include schema '%s' with mismatching targetNamespace '%s' into schema namespace '%s', assuming schema targetNamespace '%s'\n", schema.sourceLocation() ? schema.sourceLocation() : "", schema.targetNamespace, targetNamespace ? targetNamespace : "(undefined)", targetNamespace ? targetNamespace : "(undefined)");
      else
        fprintf(stderr, "\nWarning: attempt to include schema '%s' with no targetNamespace into schema namespace '%s', assuming chameleon schema targetNamespace '%s'\n", schema.sourceLocation() ? schema.sourceLocation() : "", targetNamespace ? targetNamespace : "(undefined)", targetNamespace ? targetNamespace : "(undefined)");
    }
  }
...
int xs__include::preprocess(xs__schema &schema)
{
  if (!schemaRef)
  {
    if (schemaLocation)
    {
      // only read from include locations once when not read already, uses static std::map
