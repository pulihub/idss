...
static void * instantiate_xsd__string(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
  ...
  struct soap_clist *cp = soap_new_link(soap, SOAP_TYPE_xsd__string, n, delete_xsd__string);
  QString *p = NULL;
  size_t k = sizeof(QString);
  (void)type; (void)arrayType; /* appease -Wall -Werror */
  if (!cp && soap && n != SOAP_NO_LINK_TO_DELETE)
    return NULL;
  if (n < 0)
  {
    p = SOAP_NEW(soap, QString);
  }
  else
  {
    p = SOAP_NEW_ARRAY(soap, QString, n);
    k *= n;
  }
  if (size)
    *size = k;
