...
static void * instantiate_xsd__dateTime(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
  ...
  struct soap_clist *cp = soap_new_link(soap, SOAP_TYPE_xsd__dateTime, n, delete_xsd__dateTime);
  QDateTime *p = NULL;
  size_t k = sizeof(QDateTime);
  (void)type; (void)arrayType; /* appease -Wall -Werror */
  if (!cp && soap && n != SOAP_NO_LINK_TO_DELETE)
    return NULL;
  if (n < 0)
  {
    p = SOAP_NEW(soap, QDateTime);
  }
  else
  {
    p = SOAP_NEW_ARRAY(soap, QDateTime, n);
    k *= n;
  }
  if (size)
    *size = k;
