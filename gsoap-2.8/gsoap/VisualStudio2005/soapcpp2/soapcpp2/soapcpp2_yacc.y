...
/* texpf : type expression (subset of C/C++) for templates */
texpf   : texp          { $$ = $1; }
...
type    : VOID          { $$ = mkvoid(); }
        ...
        | TYPE          {
                          ...
                          else if ($1 == lookup("std::string") || $1 == lookup("std::wstring"))
                          {
                            ...
                            if (cflag)
                          ...
                          else if ((p = entry(templatetable, $1)) ||
                              $1 == lookup("std::deque") ||
                              $1 == lookup("std::list") ||
                              $1 == lookup("std::vector") ||
                              $1 == lookup("std::set") ||
                              $1 == lookup("std::queue") ||
                              $1 == lookup("std::stack") ||
                              $1 == lookup("std::shared_ptr") ||
                              $1 == lookup("std::unique_ptr") ||
                              $1 == lookup("std::auto_ptr") ||
                              $1 == lookup("std::optional") ||
                              $1 == lookup("std::weak_ptr") ||
                              $1 == lookup("std::function"))
                          {
                            sprintf(errbuf, "at least one template type parameter T is required with %s<T>", $1->name);
                            semerror(errbuf);
                            $$ = mkint();
                          }
                          else
