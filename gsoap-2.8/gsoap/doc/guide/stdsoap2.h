...
@brief This module describes compile-time flags and functions for run-time debugging and logging

This module describes the following compile-time flags and functions to specify log files:
- `#DEBUG`
- `#SOAP_DEBUG`
- `#DEBUG_STAMP`
- `#SOAP_MEM_DEBUG`
...
/// User-definable macro, enables `#DEBUG` mode for runtime debug logs, but without enabling `#SOAP_MEM_DEBUG` memory debugging
/**
This macro can be used when the `#DEBUG` macro is reserved by the IDE for other purposes.
...
/// User-definable macro to enable memory debugging without `#SOAP_DEBUG` logging
/**
...
@brief This module describes the `WITH_MACRO` compile-time flags to configure the engine build

This module describes the following compile-time flags to configure the engine build:
- `#SOAPDEFS_H`
- `#WITH_SOAPDEFS_H`
- `#WITH_COMPAT`
- `#WITH_LEAN`
...
@brief This module describes the `SOAP_MACRO` compile-time values to configure the engine build

This module describes the following macros with values to configure the engine build:
- `#SOAP_NOTHROW`
- `#SOAP_BUFLEN`
- `#SOAP_HDRLEN`
- `#SOAP_TAGLEN`
...
@brief This module describes the `SOAP_MACRO` run-time `::soap_mode` flags to set the engine mode

This module describes the following `::soap_mode` flags:
- `#SOAP_C_MBSTRING`
- `#SOAP_C_NILSTRING`
- `#SOAP_C_NOIOB`
- `#SOAP_C_UTFSTRING`
...
@brief This module describes the `SOAP_MACRO` run-time `::soap_status` error codes returned by functions and stored in `::soap::error`
...
/// A `::soap_status` error code: heap memory overflow or corruption error, which can never happen in gSOAP unless the application code itself caused memory instability e.g. the heap is trashed
#define SOAP_MOE
...
@brief This module describes the `::soap` \ref soap "context structure with the engine state" (see its info page) with API functions to allocate, initialize, copy, and delete contexts

View the \ref soap "context structure with the engine state" (see its info page) for more details on the context structure member variables.  The member variables used in this page and elsewhere in the documentation are hyperlinked.  Most structure member variables may be changed directly as is documented with illustrative examples.
...
/// Unlink a block of heap memory allocated and managed by the specified `::soap` context, this block of heap memory should be released explicitly later by the application
/**
This function unlinks a managed block of memory from the managing `::soap` context.  This block of memory should be released explicitly later by the application logic by using `free` for regular C data types or by using `delete` for C++ objects.  Returns `#SOAP_OK` when successful or `#SOAP_ERR` when the block is not managed by the specified context.  This operation is time consuming prior to version 2.8.140 (since version 2.8.140 implements an updated memory management approach.)
*/
int soap_unlink(
    struct soap *soap, ///< `::soap` context
...
/// Delete all dynamically-allocated C++ objects managed by the specified `::soap` context (deprecated since 2.8.140 and greater, `::soap_end` also deletes managed C++ objects)
/**
...
struct soap *soap = soap_new();
... // send and receive messages etc.
soap_destroy(soap); // delete managed C++ objects (deprecated since 2.8.140 and greater, `::soap_end` also deletes managed C++ objects)
soap_end(soap);     // delete managed memory
soap_free(soap);    // free the context
~~~
...
int soap_dealloc(
  ...
  /// @returns `#SOAP_OK` when successful or `#SOAP_MOE` when a memory error is detected during deallocation
  ;

/// Explicitly dealllocates a block of managed memory that is managed by the specified `::soap` context and release the free memory back to the heap, this function is idential to the `::soap_dealloc` function
int soap_delete(
    struct soap *soap, ///< `::soap` context
    void *ptr)         ///< pointer to the block of managed heap memory to deallocate
  /// @returns `#SOAP_OK` when successful or `#SOAP_MOE` when a memory error is detected during deallocation
...
Normally returns `#SOAP_OK`.  Returns `#SOAP_MOE` when a memory error is detected, which should never happen; note that memory leaks, double frees, and overruns are always reported by `::soap_done` when compiled with `#DEBUG` or `#SOAP_MEM_DEBUG`.
...
@see `::soap_malloc`, `::soap_destroy`.
*/
int soap_end(struct soap *soap) ///< `::soap` context
  /// @returns `#SOAP_OK` when successful or `#SOAP_MOE` when a memory error is detected
  ;
...
@brief This module describes the callback functions of the `::soap` context to modify its behavior, as is done by plugins
...
@brief This module describes functions to set the SSL/TLS context for HTTPS and WS-Security
...
@brief This module describes functions for HTTP operations, functions for data transmission, and plugins

This module describes the following client-side functions:
- `::soap_GET`
- `::soap_PUT`
- `::soap_PATCH`
- `::soap_POST`
...
This module describes the following server-side functions:
- `::soap_bind`
- `::soap_accept`
...
This module describes the following input/output functions:
- `::soap_begin_recv`
- `::soap_end_recv`
- `::soap_begin_send`
- `::soap_end_send`
...
This module describes the following SOAP message input/output functions:
...
This module describes the following HTTP-related plugins;
...
- `::http_form` HTTP FORM plugin for server-side handling of HTTP application/x-www-form-urlencoded data.
...
/// The HTTP FORM plugin registration function
/**
This function is used to register the `::http_form` HTTP FORM plugin to handle application/x-www-form-urlencoded data with `soap_register_plugin_arg(soap, http_form, http_form_handler)` where the `http_form_handler` is a function to parse the application/x-www-form-urlencoded data received by the server.  The HTTP FORM plugin API is declared and defined in <i>`gsoap/plugin/httpform.h`</i> and <i>`gsoap/plugin/httpform.c`</i>.
...
@brief This module describes functions to set and get HTTP cookies at the server side
...
@brief This module describes the conversion functions of values of various types to and from strings
...
@brief This module describes the `::Namespace` XML namespace structure and function to activate a table
...
@brief This module describes the `::SOAP_ENV__Header` structure and `::soap_header` function to allocate the header
...
@brief This module describes the `::SOAP_ENV__Fault` structure and functions to set and get fault information
...
@brief This module describes functions to set and get DIME attachments
...
@brief This module describes functions to set and get MIME/MTOM attachments
...
@brief This module describes plugin registry functions to register plugins

Available plugins:
- `::logging` plugin
- `::http_get` HTTP GET plugin
- `::http_post` HTTP POST plugin
- `::http_form` HTTP FORM plugin
- [HTTP streams plugin](../../streams/html/httpstreams.html)
...
@brief This module describes portable thread and mutex functions
...
/// Mutex object try lock attempt to lock
/**
This macro acquires mutex if not locked, returns zero on success and nonzero on failure to lock
...
@brief This module describes other useful functions
