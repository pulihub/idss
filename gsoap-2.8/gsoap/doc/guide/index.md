...
<div align="right"><i>Copyright (c) 2000-2026, Genivia Inc.<br>All rights reserved.</i></div>
...
* libssl-dev 3.0 and OpenSSL 3.0 or the latest (OpenSSL 1.1 and earlier
  versions are also supported but not recommended).  Alternatively, GNUTLS or
  WolfSSL are also supported.

* Optionally libz to enable message compression. This library is
  typically installed on most platforms.
...
for FastCGI. Compile from source <i>`gsoap/stdsoap2.c`</i> (or <i>`gsoap/stdsoap2.cpp`</i>)
instead.
...
### DOM API extension overview {#dom}
...
### The HTTP streams plugin   {#streamsplugin}

The plugin code is located in the <i>`gsoap/plugin`</i> directory containing
<i>`httpstreams.h`</i> and <i>`httpstreams.c`</i>, including an example
<i>`httpstreamstest.c`</i>.

For more details, see the [HTTP streams plugin](../../streams/html/index.html) documentation.
...
<i>Copyright (c) 2000-2026, Robert A. van Engelen, Genivia Inc.<br>All rights reserved.</i>
